"""
Step 9: Evaluate Hybrid GTCN model (Setup 2) on withdrew participants.

Same pipeline as withdrew_general_eval but uses the hybrid model.
Also runs a random baseline simulation for comparison.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mc_iterations", type=int, default=50,
                        help="Monte Carlo iterations for random baseline (default: 50).")
...
def main() -> None:
    ...
    print(f"Loaded medians: {medians_csv_path}")
    global_means = pd.read_csv(global_means_csv_path)
    print(f"Loaded global means: {global_means_csv_path}")
    ...
    df_breaking_points.to_csv(
        os.path.join(args.output_dir, "withdrawn_user_thresholds_s2.csv"), index=False
    )
    ...
    ax.set_title("Distribution of Burden Reduction Rates (Setup 2 Model)", fontsize=16)
    ax.set_xlabel("Burden Reduction Rate (%)", fontsize=14)
    ax.set_ylabel("Number of Participants", fontsize=14)
    ax.legend()
    ax.grid(axis="y", alpha=0.75)
    save_figure(fig, args.output_dir, "hybrid_rnn_burden_reduction_distribution.png")

    # Calculate study extension
    df_extension = calculate_study_extension(df_breaking_points, df_sim, model_name="Setup 2")
    df_extension.to_csv(
        os.path.join(args.output_dir, "withdrawn_user_study_extension_setup2.csv"), index=False
    )
    ...
    save_text_results(extension_text, args.output_dir, "hybrid_rnn_study_extension_results.txt")
    ...
    ax2.set_title("Distribution of Projected Study Extensions (Setup 2)", fontsize=16)
    ax2.set_xlabel("Projected Study Extension (days)", fontsize=14)
    ax2.set_ylabel("Number of Participants", fontsize=14)
    ax2.legend()
    save_figure(fig2, args.output_dir, "hybrid_rnn_study_extension_distribution.png")
    ...
    # --- Random Baseline ---
    print("\n" + "=" * 50)
    print("Running Random Baseline Simulation")
    print("=" * 50)

    df_sim_random = simulate_random_baseline(
        df_breaking_points, Y_withdrawn, p_ids,
        block_rate=args.block_rate, iterations=args.mc_iterations
    )
    df_sim_random.to_csv(
        os.path.join(args.output_dir, "withdrawn_user_random_baseline_simulation.csv"), index=False
    )
    ...
    print(random_text)
    save_text_results(random_text, args.output_dir, "random_baseline_results.txt")

    fig4 = plot_actual_vs_projected_density(
        df_sim_random, title="Actual vs Projected Days (Random Baseline)"
    )
    save_figure(fig4, args.output_dir, "random_baseline_actual_vs_projected_density.png")

    print("\nSetup 2 + Random Baseline evaluation complete.")
