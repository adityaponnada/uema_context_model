...
def main() -> None:
    ...
    if os.path.exists(args.heldout_csv):
        ...
        df_importance.to_csv(os.path.join(args.output_dir, "hybrid_rnn_feature_importance.csv"), index=False)

        top10 = df_importance.head(10)
        top10_text = (
            "Top 10 Features by Permutation Importance for Hybrid GTCN\n"
            f"{'=' * 60}\n\n"
        )
